...
#define LORA_SPREADING_FACTOR                       11         // 7[SF7..SF12]    Define o fator de espalhamento do LoRa
...
const unsigned long BOT_MTBS = 1500; // mean time between scan messages
...
void setup() {
    ...
    Mcu.begin(3,0);  // Inicializa a MCU (Unidade de Microcontrole).
...
void handleMenu(const String& chatId) {
  ...
  message += "/distancia - exibe a distância do cuidador para o paciente\n";
  message += "/local_paciente - exibe a localização do paciente\n";
  message += "/menu - Show the command menu";
  bot.sendMessage(chatId, message, "Markdown"); // Enviando a mensagem aqui
}
...
void handleNewMessages(int numNewMessages)
{
  ...
  for (int i = 0; i < numNewMessages; i++)
  {
    ...
    if (comando == "/distancia")
      answer = "A DISTÂNCIA ATUAL É " + String(distance) + " METROS";
    else if (comando == "/local_paciente")
      answer = "Link do google maps: " + local;
...
void bot_setup()
{
  const String commands = F("["
                            "{\"command\":\"distancia\", \"description\":\"Exibe a distância entre o cuidador e o paciente\"},"
                            "{\"command\":\"local_paciente\", \"description\":\"Exibe a localização do paciente\"},"
                            "{\"command\":\"menu\",  \"description\":\"Exibe menu de comandos\"}" // no comma on the last command
                            "]");
